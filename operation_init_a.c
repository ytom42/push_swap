...
/*   Updated: 2021/12/19 10:46:57 by ytomiyos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	operation_init_a(t_stacks *s)
{
	...
	while (i <= s->a_top)
	{
		...
		if (index_of_bottom)
		{
			if ((s->b_top - index_of_bottom) == s->b_top)
				element->rb = 0;
			else
				element->rb = s->b_top - index_of_bottom;
			if (index_of_bottom == s->b_top)
				element->rrb = 0;
			else
				element->rrb = index_of_bottom;
			element->exec_num = get_exec_num(s, i, 'a');
		}
		else if (index_of_top)
		{
			if ((s->b_top - index_of_top + 1) == s->b_top)
				element->rb = 0;
			else
				element->rb = s->b_top - index_of_top + 1;
